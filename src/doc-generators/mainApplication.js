...
export default ([step_0, step_1, step_2, step_3, step_4, step_5, step_6], documentCreatorInitializer) => {
    ...
    li(nb, 'odpisu aktu urodzenia – na fakt oznaczenia mojej płci w akcie urodzenia jako '+({K: 'męskiej;', M: 'żeńskiej;'}[a.new_mark] || '.......;'), {
        level: 2,
    })
    ...
    if (a.psychologist) {
        li(nb, 'opinii psychologa – na fakt stwierdzenia u mnie trwałej identyfikacji z płcią '+part1+', wykluczenia wtórnego pochodzenia niezgodności płciowej, postawienia mi formalnej diagnozy '+diagnosis(a.psychologist_f64, a.psychologist_ha60), {
            level: 2,
        })
    }
    if (a.psychologist_sexologist) {
        li(nb, 'opinii psychologa-seksuologa – na fakt stwierdzenia u mnie trwałej identyfikacji z płcią '+part1+', wykluczenia wtórnego pochodzenia niezgodności płciowej, postawienia mi formalnej diagnozy '+diagnosis(a.psychologist_sexologist_f64, a.psychologist_sexologist_ha60), {
            level: 2,
        })
    }
    if (a.psychiatrist) {
        li(nb, 'zaświadczenia lekarza psychiatry – na fakt występowania u mnie trwałej identyfikacji z płcią '+part1+', wykluczenia wtórnego pochodzenia niezgodności płciowej, postawienia mi formalnej diagnozy '+diagnosis(a.psychiatrist_f64, a.psychiatrist_ha60), {
            level: 2,
        })
    }
    if (a.sexologist) {
        li(nb, 'zaświadczenia lekarza seksuologa – na fakt występowania u mnie trwałej identyfikacji z płcią '+part1+', wykluczenia wtórnego pochodzenia niezgodności płciowej, postawienia mi formalnej diagnozy '+diagnosis(a.sexologist_f64, a.sexologist_ha60), {
            level: 2,
        })
    }
    ...
    if (a.proving_documents) {
        li(nb, ({ A: 'wydruku z portali społecznościowych', B: 'plakietki identyfikacyjnej z miejsca pracy', C: 'wydruku z portalu USOS' }[a.proving_documents_type] || '......................')+' – na fakt występowania u mnie trwałej identyfikacji z płcią '+part1+', funkcjonowania jako '+({ K: 'kobieta', M: 'mężczyzna' }[a.new_mark] || '.......')+' w otoczeniu;', {
            level: 2,
        })
    }
    ...
    let text = 'Na wstępie wyjaśniam, że zdaję sobie sprawę z metrykalnego oznaczenia mojej płci jako '+({ K: 'męskiej', M: 'żeńskiej' }[a.new_mark] || '.......')+', jednak wskazuję, że w codziennym życiu funkcjonuję jako '+({ K: 'kobieta', M: 'mężczyzna' }[a.new_mark] || '.......')+'. W związku z tym, że tożsamość płciowa jest jednym z dóbr osobistych człowieka, we wniosku będę używać '+({ K: 'żeńskich', M: 'męskich' }[a.new_mark] || '.......')+' form gramatycznych.'
    if (a.has_common_name) {
        text += ' Jednocześnie wskazuję, że na co dzień używam imienia '+ (normalize(a.common_name) || '....... ') +'.'
    }
    p(text, {
        spaceBefore: 6,
        spaceAfter: 8,
    ...
    text += ' zarejestrował moje urodzenie w dniu '+(a.birth_date ? (new Date(a.birth_date)).toLocaleDateString('pl-PL', { dateStyle: 'long' }) : '........')
    text += ' w akcie o numerze '+(normalize(a.birth_certificate_id) || '..................... ')+'. '
    text += 'Moja płeć została tam oznaczona jako '+({ K: 'męska', M: 'żeńska' }[a.new_mark] || '.......')+', w oparciu o ocenę mojej budowy anatomicznej przez personel medyczny. '
    ...
    text = 'W związku z głęboko przeżywanym poczuciem identyfikacji z płcią '+({ K: 'żeńską', M: 'męską' }[a.new_mark] || '.......')+' i występującą jednocześnie dysforią, '+({ K: 'rozpoczęłam', M: 'rozpocząłem' }[a.new_mark] || 'rozpocz_ł_m')+' formalną diagnostykę u lekarzy specjalistów, w wyniku której '+({ K: 'otrzymałam', M: 'otrzymałem' }[a.new_mark] || 'otrzymał_m')+' diagnozę '
    ...
    text += ', zgodnie z wytycznymi Polskiego Towarzystwa Seksuologicznego. W toku tego procesu przekazano mi wszystkie informacje niezbędne do wyrażenia przeze mnie świadomej zgody na wdrożenie leczenia hormonalnego, a także wykluczono wtórne (np. wynikające z zaburzeń psychicznych) pochodzenie '
    if (isAnyF64 && isAnyHA60) {
        text += 'dysforii płciowej / niezgodności płciowej'
    } else if (isAnyHA60) {
        text += 'niezgodności płciowej'
    } else {
        text += 'dysforii płciowej'
    }
    text += '. Specjaliści przeprowadzili podmiotowe badania psychologiczne'
    ...
    p('Od '+(a.hrt_since ? ((['stycznia', 'lutego', 'marca', 'kwietnia', 'maja', 'czerwca', 'lipca', 'sierpnia', 'września', 'października', 'listopada', 'grudnia'][+a.hrt_since[0] - 1] || '') + ' ' + (a.hrt_since[1] || '.......')) : '......................')+' wdrożono u mnie leczenie hormonalne. To oznacza, że przyjmuję hormony, których celem jest '+({ K: 'feminizacja', M: 'maskulinizacja' }[a.new_mark] || '......................')+' mojego ciała. Zmiany, które temu towarzyszą, są przeze mnie odbierane pozytywnie.')
    p('W sferze społecznej '+({ some: 'w niektórych obszarach', all: 'w większości obszarów' }[a.areas] || '......................')+' funkcjonuję zgodnie z moją '+({ K: 'żeńską ', M: 'męską ' }[a.new_mark] || '')+'tożsamością płciową. Moja rodzina, osoby bliskie, koledzy i koleżanki, '+(a.has_common_name ? ('znają mnie jako '+a.common_name+' i ') : '')+'używają wobec mnie '+({ K: 'żeńskich', M: 'męskich' }[a.new_mark] || '.......')+' form gramatycznych.')
    p('Brak zmiany oznaczenia płci'+(a.is_new_firstname ? ' oraz imienia' : '')+' powoduje u mnie duże trudności w codziennym funkcjonowaniu. We wszystkich przypadkach w których muszę używać danych zawartych w akcie urodzenia lub okazywać dowód tożsamości, moja tożsamość jest kwestionowana z uwagi na wygląd, odpowiadający typowym wyobrażeniom o '+({ K: 'kobiecym', M: 'męskim' }[a.new_mark] || '.......')+' wyglądzie. Zmusza mnie to też do ujawniania osobom postronnym, że jestem osobą transpłciową, co głęboko ingeruje w moją prywatność i pozbawia mnie szansy na decydowanie o tym, kto będzie wiedzieć o mojej transpłciowości. Brak zmiany oznaczenia płci pozbawia mnie więc sprawczości i decyzyjności w jednym z kluczowych aspektów mojego życia. Co więcej, brak zmiany danych wpływa też na możliwość podjęcia dalszych kroków w tranzycji medycznej. Utrudnia też znalezienie i utrzymanie pracy. Osoby transpłciowe są grupą najczęściej dyskryminowaną na rynku pracy wśród społeczności LGBT+. Dopóki moje dane metrykalne są inne niż zgodne z moją tożsamością płciową, jestem nieustannie zmuszona do ujawniania pracodawcom swojej tożsamości, co obniża szansę na bycie '+({ K: 'zatrudnioną.', M: 'zatrudnionym.' }[a.new_mark] || 'zatrudnion_.'))
    p('Powyższe okoliczności wskazują jednoznacznie, że w moim przypadku poczucie przynależności do płci '+({ K: 'żeńskiej', M: 'męskiej' }[a.new_mark] || '.......')+' jest trwałe i że uwzględnienie niniejszego wniosku jest uzasadnione.')
    ...
    if (a.pts_guidelines) attachments.push('dokument zatytułowany |Zalecenia Polskiego Towarzystwa Seksuologicznego dotyczące opieki nad zdrowiem dorosłych osób transpłciowych – stanowisko panelu ekspertów|')
